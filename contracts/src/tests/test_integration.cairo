...
// ═══════════════════════════════════════════════════════════════════════════
// 5.4 — Multiplayer: two explorers in the same temple
// ═══════════════════════════════════════════════════════════════════════════

    /// Explorer A opens an exit; Explorer B (in the same chamber) can
    /// move through it without opening it again.
    #[test]
    ...
    /// Explorer A kills a monster (simulated via write_model_test);
    /// Explorer B enters the same chamber and does NOT enter combat
    /// because the shared MonsterInstance.is_alive is false.
    #[test]
    ...
    /// Explorer A loots treasure; Explorer B in the same chamber
    /// cannot loot again (treasure_looted is shared).
    #[test]
    #[should_panic]
    ...
    /// Explorer A disarms a trap; Explorer B enters the same chamber
    /// and takes no trap damage (trap_disarmed is shared).
    #[test]
    ...
    /// Both explorers can read the same monster HP. Simulate A dealing
    /// damage (via write_model_test), then B reads and sees reduced HP.
    /// This verifies MonsterInstance is shared state, not per-explorer.
    #[test]
    ...
    /// Explorer A opens exit 0 generating chamber 2. Explorer B
    /// opens exit 1 generating chamber 3. Both chambers co-exist
    ...
    /// Both explorers are in the same chamber. One dies and the other
    /// can see and loot the body, verifying ChamberFallenCount is shared.
    #[test]
    ...
    /// Each explorer tracks their own progress independently within
    /// the same temple (chambers_explored, xp_earned are per-explorer).
    #[test]
...
// ═══════════════════════════════════════════════════════════════════════════
// 5.5 — Boss probability: Yonder Formula distribution tests
// ═══════════════════════════════════════════════════════════════════════════

    // Mirror of the contract's calculate_boss_probability for testing.
    // Formula: effective_yonder² × 50 + xp_earned × 2, capped at 9500 bps.
    // Returns 0 if yonder < 5.
    ...
    /// Yonder < 5 always returns 0% — no boss before depth 5.
    #[test]
    ...
    /// At yonder=5, effective_yonder=0, so only XP component matters.
    #[test]
    ...
    /// Quadratic yonder growth: effective_yonder² × 50 bps.
    #[test]
    ...
    /// Combined yonder + XP components add linearly.
    #[test]
    ...
    /// Cap at 9500 bps (95%) — never reaches 100%.
    #[test]
    ...
    /// Verify exact bps values at key progression milestones.
    /// These represent a typical player's journey through the temple.
    #[test]
    ...
    /// The VRF roll in generate_chamber scales d20 (1-20) to bps (500-10000).
    /// Verify that the roll range aligns with the probability calculation.
    #[test]
