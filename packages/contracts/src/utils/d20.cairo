...
use d20::utils::vrf::{
    IVrfProviderDispatcher, IVrfProviderDispatcherTrait, Source, VRF_PROVIDER_ADDRESS,
};
...
pub fn roll_d20(caller: ContractAddress) -> u8 {
...
/// Roll `count` dice each with `sides` faces (e.g. roll_dice(caller, 6, 2) = 2d6).
/// Returns the sum of all dice.
pub fn roll_dice(caller: ContractAddress, sides: u8, count: u8) -> u16 {
    let vrf_provider = IVrfProviderDispatcher {
        contract_address: VRF_PROVIDER_ADDRESS.try_into().unwrap(),
    };
    let mut total: u16 = 0;
